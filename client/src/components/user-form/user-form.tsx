    ...
    <form className="w-full max-w-sm">
      ...
      <div className="md:flex md:items-center mb-6">
        <div className="md:w-1/3">
          ...
          >
            <img src={config.ASSETS[config.ENVIRONMENT] + `avatars/${user.avatar}`} className="w-8 h-8 rounded-full float-right" />
          </label>
        </div>
      ...
      <div className="md:flex md:items-center mb-6">
        ...
        <div className="md:w-2/3">
          <input
            ...
            value={user.email}
            onInput={(event: React.ChangeEvent<HTMLInputElement>) => setUser(prevUser => ({ ...prevUser, email: event.target.value}))}
          />
        </div>
      </div>
